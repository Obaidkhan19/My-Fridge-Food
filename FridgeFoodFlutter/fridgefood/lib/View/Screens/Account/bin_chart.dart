...
import 'package:fridgefood/View/Screens/Account/bin_logs.dart';
...
class _BinDashboardState extends State<BinDashboard> {
  ...
  // GET Bins
  List<Chart> binlist = [];
  Future<List<Chart>> binchartapi() async {
    final response =
        await http.get(Uri.parse('$ip/log/ChartBinLogs?fid=$fridgeid'));
    var data = jsonDecode(response.body.toString());
    if (response.statusCode == 200) {
      binlist.clear();
      // sucess
      for (Map i in data) {
        binlist.add(Chart.fromJson(i));
      }
      return binlist;
    } else {
      return binlist;
